...
class Createjob extends MX_Controller
{
    ...
    public function index()
    {
        ...
        $data['content']      = $this->load->view('createjob/createJob', $data, TRUE);
    ...
    public function cancel_job() {
        ...
        $this->db->where('id',$id); 
        $this->db->update("hbp_jobs",array("status"=>"3"));
    ...
    public function contractor_detail()
    {
        ...
        // print_r($data['result'] );
        // die;
        $data['recent']     = $this->jobs_model->recent_jobs($id);
/* 		echo "<pre>";
		print_r($data['recent']);
		die; */
   ...
   /* public function update()
    ...
    public function fetch_table()
    {
      ...
      //die();
        ...
        foreach ($data as $row) {
            ...
            $tbl .= '<tr>
               <td><div><img class="img-circle" id="circle" src="' . base_url() . 'uploads/' . $row->profile_pic . '"></div><div class="bidder_property"><a href="' . base_url() . '/homeowner/jobs/contractor_detail?id=' . $row->contractor_id . '&job_id=' . $row->job_id . '">' . $row->contractor_name . '</a></div></th>
                ...
                <td><a href= "'.$payment_url.'?bid_id='.$row->job_bid_id.'&'.'job_id='.$id.'" style="padding:0px;" data="' . $row->job_bid_id . '" class="status_checks btn ' . (($row->status) ? 'a' : 'b') . '">' . (($row->status) ? '<button class="btn btn-default btn-xs button active" type="button">Accept </button> ' : '<button class="btn btn-default btn-xs button"  type="button">Accept </button>') . '</a>
                </td>
        ...
        $tbl .= '<tr>  <td style ="color:red;">Sorry No Results </td></tr>'  ;
    ...
    function do_upload()
    {
        ...
        $data              = array(
            ...
            'counties' => $countiesForSearch,
            'project_discription' => $this->input->post('description')
        );
        
        // echo "<pre>";   print_r($data);die;
         $this->session->set_userdata('jobData', $data);
         $session_data = $this->session->userdata('jobData');
    
        if($session_data)
        {
            //echo "<pre>";print_r($session_data);die;
            $this->session->set_flashdata('homeownerFlash', 'You have successfully created a job,if you are new user then sign up please');                       
        ...
        /*  echo "<pre>"; print_r($session_data);die;
          ...
          $insert_id  =  $this->db->insert_id();
            ...
            foreach ($categoryForSearch as $key => $cat) {
                       ...
                       $data1 =  array(
                        ...
                        $insertData12 = $this->db->insert('hbp_jobs_categories',$data1);
