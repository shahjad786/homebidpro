...
echo $percent;?>
    ...
    ?>  
    <a href="<?php echo base_url();?>homeowner/jobs/cancel_job?job_id=<?php $job_id=$_GET['job_id']; echo $job_id;?>"><button class="btn btn-default btn-sm pull-right bids disabled_button" style="color:black;" disabled = "disabled" type="button" title = "You can't cancel because it accepted">CANCEL PROJECT</button></a>   

 <?php
  }

else  { ?>  
      <a  onclick ="return ConfirmDialog();" href="<?php echo base_url();?>homeowner/jobs/cancel_job?job_id=<?php $job_id=$_GET['job_id']; echo $job_id;?>"><button class="btn btn-default btn-sm pull-right bids enabled_button" type="button">CANCEL PROJECT</button></a>
...
<?php } ?>
 ...
 $payment_url = base_url()."homeowner/payment/";
  foreach($check_bidder_status as $data);
    ...
    echo '<a href= "'.$payment_url.'?bid_id='.$row->job_bid_id.'" style="padding:0px;" data="' . $row->job_bid_id . '" class="status_checks btn ' . (($row->status) ? 'a' : 'b') . '">' . (($row->status) ? '<button class="btn btn-default btn-sm pull-right button active disabled_button" disabled="disabled" title = "This project is already accepted" type="button">ACCEPT BID </button> ' : '<button class=btn btn-default btn-sm pull-right button" title = "This project is already accepted" type="button">ACCEPT BID </button>') . '</a>';
        }

else  {   
      echo '<a href= "'.$payment_url.'?bid_id='.$row->job_bid_id.'" style="padding:0px;" data="' . $row->job_bid_id . '" class="status_checks btn ' . (($row->status) ? 'a' : 'b') . '">' . (($row->status) ? '<button class="btn btn-default btn-sm pull-right button active enabled_button"  type="button">ACCEPT BID </button> ' : '<button class=btn btn-default btn-sm pull-right button"  type="button">ACCEPT BID </button>') . '</a>';
...
<img src="<?php echo base_url();?>media/img/bid_detail_about.jpg">
</div>
<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
...
</fieldset> <h5><b><?php echo $rev->job_name  ;?>  </b> <?php echo date('F d, Y', strtotime($rev->created_at)) ?> <img  src="<?php echo base_url();?>media/img/review_img.jpg"></h5>
                                  <?php echo $rev->reviews;?> <h6 class="name"><b><?php echo $rev->owner_name;?></b></h6>
                              </div>
                         </div>
...
<div id="tutorial-<?php echo $recent_job->id; ?>">
<input type="hidden" name="rating" id="rating" value="<?php echo $recent_job->rating; ?>" />
<ul onMouseOut="resetRating(<?php echo $recent_job->id; ?>);">
  <?php
  for($i=1;$i<=5;$i++) {
  $selected = "";
...
</fieldset><br /> <h5><b><?php echo $recent_job->job_name; ?>  </b> <?php echo date('F d,Y', strtotime($recent_job->created_at)); ?> <img src="<?php echo base_url();?>media/img/review_img.jpg"></h5>
        </div>
